...
pub const NativeFileDialog = struct {
    ...
    pub fn setDirectory(self: *NativeFileDialog, dir: [:0]const u8) void {
    ...
    pub fn setFilter(self: *NativeFileDialog, f: [:0]const u8) void {
        c.Fl_Native_File_Chooser_set_filter(self.inner, f);
    }

    /// Sets the preset filter for the dialog
    pub fn setPresetFile(self: *NativeFileDialog, f: [:0]const u8) void {
